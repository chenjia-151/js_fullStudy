  ...
  methods: {
    today () { // 默认选中当天，并初始化组件
      let DATE = new Date(),
          ...
          month = DATE.getMonth() + 1,   // getMonth()  取出的月份是1到11，在后面加1
          date = DATE.getDate(),
    ...
    createDays(year, month) {
      ...
      for (let i = 1; i <= days; i++) {
        thisMonthDays.push({
          ...
          week: this.data.weekText[new Date(Date.UTC(year, month - 1, i)).getDay()]   // 获取到某一年某一月某一天的星期几
        })
      }
      this.setData({
        thisMonthDays
    ...
    getThisMonthDays(year, month) {
      return new Date(year, month, 0).getDate()   // 获取当前这个月有多少天
    },
